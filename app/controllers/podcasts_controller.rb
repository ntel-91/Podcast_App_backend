...
class PodcastsController < ApplicationController
    ...
    def podcastdata
        ...
        episodes = podcast_episodes.map do |e| 
            ({
                episode_name: e.children.at_css('title').content,
    ...
    def create
        podcast = Podcast.find{|p| p.podcast_name ==  params[:podcast_name]}

        if podcast
            Subscription.create(podcast_id: podcast.id, user_id: params[:user_id])
        else
            newPodcast = Podcast.create(podcast_name: params[:podcast_name], img_url: params[:podcast_image], rss: params[:rss])
            Subscription.create(podcast_id: newPodcast.id, user_id: params[:user_id])
        end
